            ...
            {selected.map((option) => {
              return (
                <div
                  key={option.value}
                  className={cn(
                    "bg-secondary hover:bg-secondary/50 text-secondary-foreground animate-fadeIn relative inline-flex h-7 cursor-default items-center rounded-md border border-solid pe-7 pl-2 ps-2 text-xs font-medium transition-all disabled:cursor-not-allowed disabled:opacity-50 data-[fixed]:pe-2",
                    badgeClassName,
                  )}
                ...
                >
                  {option.label}
                  <button
                    className="cursor-pointer text-secondary-foreground absolute -inset-y-px -end-px flex size-7 items-center justify-center rounded-e-lg border border-transparent p-0 outline-0 transition-colors hover:text-secondary focus-visible:outline focus-visible:outline-ring/70"
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
