...
export const editMemberToOrganization = authActionClient
  .metadata({ actionName: "editMember" }) 
  .schema(z.object({
    organizationId: z.string(),
    userId: z.string(),
    role: z.enum(Object.values(MemberRole) as [string, ...string[]])
  }))
  .action(async ({ parsedInput: { userId, organizationId, role } }) => {

  try {
    // New organization
    await prisma.usersOnOrganizations.updateMany({
      where: {
        userId: userId,
...
export const deleteMemberToOrganization = authActionClient
  .metadata({ actionName: "deleteMember" }) 
  .schema(z.object({
    organizationId: z.string(),
    userId: z.string()
  }))
  .action(async ({ parsedInput: { userId, organizationId } }) => {
